...
export default function BudgetPanel() {
  ...
  const fetchData = () => {
    ...
    fetch("/api/budget/details").then(r => r.json()).then(setDetails).catch(() => {});
    fetch("/api/budget/config").then(r => r.json()).then(d => { setConfig(d); setForm({ maxSessionTokens: d.maxSessionTokens, maxDailyTokens: d.maxDailyTokens, maxSessionCostUsd: d.maxSessionCostUsd || 2, maxDailyCostUsd: d.maxDailyCostUsd || 5, warningThreshold: d.warningThreshold || 0.8 }); }).catch(() => showToast("Failed to load budget config", "error"));
  };
